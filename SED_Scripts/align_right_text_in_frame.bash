...
# command line argument. The script prints the text right aligned in a
# frame. The character of the frame and the frame width can be optionally
...
1 {x;{s/^.*$/'"${CHR}"'/;: count;/^.\{1,'"${LINE_WIDTH}"'\}$/ {s/^\(.*\)$/'"${CHR}"'\1/
b count};s/.$//p};x}
