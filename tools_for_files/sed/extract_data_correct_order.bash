...
# delimiter e.g. a semicolon ; the data is extracted from the line and
# printed out line by line. Empty lines are ignored and if there is no
# valid data the field is ignored too.
#
...
s/${PAT}$//                                     # If last char is the pattern remove it.
...
s/^${PAT}[[:blank:]]*\(.*$\)/\1/                # Remove pattern and spaces if exiting.
