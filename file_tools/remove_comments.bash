...
# start with a hashtag #. Inline comments must be preceded by a space before
# the hashtag # for the inline comment to be valid as such. Regular comments
# are recognised with or without indentation.
#
# Explanation:
# First line is checked on the existence of a Shebang which is always
# introduced by '#!/' followed by different possible paths. If there is a
# Shebang, such a line is not a comment line. Regular comments are identified
# with or without indentation and than deleted. Inline comments starts with
# one or more space followed by a hashtag # with characters up to the end of
# the line and removed.
#
# Functionality Check:
...
#
# Limitation:
# If a hashtag # is found between single or double quotes the inline comment
# will not be touched.
#
# To-Do:
...
# '/\x22.*#\+.*\x22/b' & '/\x27.*#\+.*\x27/b' -> Match hashtag # between quotes
# 's/\(.*[[:space:]]#\).*$/\1/g;s/[[:space:]]\+[#]$//' -> Delete inline comments
sed -i -e '0,/^#!\//p' -e '/^[[:blank:]]*#/d' \
       -e '/\x22.*#\+.*\x22/b' -e '/\x27.*#\+.*\x27/b' \
       -e 's/\(.*[[:space:]]\+#\).*$/\1/g;s/[[:space:]]\+[#]$//' \
